...
{
    if (arg_num > 3) {
        raise_error("font：引数が多すぎます@@ %d個渡されました", arg_num);
    }
    if (arg_num <= 0) {
        raise_error("font：引数がたりません");
    }
    char* name = (char*)"";
    int size = font_size;
    int style = 16;
...
{
    if (arg_num <= 0) {
        raise_error("picload：引数がたりません");
    }
    if (arg_num > 1) {
        raise_error("picload：引数が多すぎます@@ %d個渡されました", arg_num);
    }
    const auto m = stack_peek(s->stack_);
    value_isolate(*m);
    if (m->type_ != VALUE_STRING) {
        ...
        static void dump(int indent, ast_node_t* node, bool is_detail)
        {
            ...
            if (node->ext_) {
                if (node->tag_ == NODE_IF_CHECK) {
                    dump(indent + 1, reinterpret_cast<ast_node_t*>(node->ext_), is_detail);
                }
            }
        }
    };
...
dump_variable(list_t* var_table, const char* name, int idx)
{
    ...
    if (auto v = search_variable(var_table, name)) {
        ...
        switch (v->type_) {
            case VALUE_INT:
                printf("%d", reinterpret_cast<int*>(p)[0]);
                break;
            case VALUE_DOUBLE:
                printf("%lf", reinterpret_cast<double*>(p)[0]);
                break;
            case VALUE_STRING:
                printf("%s", reinterpret_cast<char*>(p));
                break;
            default:
                assert(false);
                break;
